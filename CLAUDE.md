...
- **`ARCHITECTURE.md`** —— 日常修改入口图：改什么文件、怎么验证、哪些边界不能碰
...
- 当前仓库是独立博客仓库：`git@github.com:a444447/a444447.github.io.git`
- 当前公开站点目标：`https://a444447.github.io`
...
| 域名 | **暂无自定义域名**，使用 `a444447.github.io` | 未来可迁移 |
...
├── tsconfig.json
├── package.json
├── ARCHITECTURE.md                ← 日常修改指南
├── migration-report.md            ← Hugo 旧文迁移报告
├── scripts/
│   └── migrate-hugo-posts.mjs     ← Hugo → Astro 内容迁移脚本
├── public/
...
│   │   └── site.ts                ← 站点 URL、作者、RSS、OG 等全局常量
│   └── styles/
...
- 现在：`site: 'https://a444447.github.io'`
- 未来换域名时改这一行，其它地方都能自动同步
- **不需要 `base` 字段**（用户站部署到根路径）

### 当前部署事实

- GitHub 远端：`git@github.com:a444447/a444447.github.io.git`
- 默认分支：`main`
- Pages 类型：用户站，根路径部署
- 本地构建命令：`npm run build`
- 本地预览命令：`npm run preview`
- GitHub Actions 使用 Node 20；本地当前兼容 Node 18.13+
- 首次部署前仍需在仓库 `Settings → Pages → Build and deployment → Source` 选择 `GitHub Actions`
...
- 当前按作者决定使用 **Expressive Code 默认主题**，不启用站点自定义主题 override
- 使用它**自带的复制按钮插件**，不自己写 island
- 旧文迁移时会把 Shiki 不支持的代码块语言降级或规范化，例如 `c++` → `cpp`、`llvm` → `text`
- 如果未来重新启用主题 override，必须先和 `style-specimen.html` 对齐，并明确这是一个新的视觉迭代
...
- 每页 `<head>` 由 `BaseLayout.astro` 统一输出 canonical、RSS、sitemap、`og:*` 和 `twitter:*`
- **策略 A→B**：站点默认 OG 图，文章有 `cover` 时用 `cover`
- 策略 C（构建期 Satori 生成独特 OG）**暂不做**

### Pagefind 搜索

- 搜索组件是 `src/components/islands/SearchBox.tsx`
- `BaseLayout.astro` 中使用 `<SearchBox client:idle />`
- Pagefind 不参与 `npm run dev`，只在 `npm run build` 后扫描 `dist/`
- 404 页面通过 `pagefind={false}` 不进入搜索索引

### Hugo 旧文迁移

- 迁移源：`/Users/afourseven/blog/content/posts`
- 迁移脚本：`scripts/migrate-hugo-posts.mjs`
- 迁移报告：`migration-report.md`
- 已迁移 50 篇正式文章到 `src/content/posts`
- 已跳过草稿、缺 `title`/`date` 的文章、Hugo/shortcode 说明文章
- Hugo shortcode 不再重建 shortcode 系统，统一转换为普通 Markdown 或原生 HTML
...
#### 代码块（已暂停的 Expressive Code 主题目标）

当前实现**没有启用**下面这套 override。代码块使用 Expressive Code 默认样式，保留复制按钮。以下内容只作为未来视觉精修参考，不能当作当前实现事实。
...
- ✅ Hugo 旧文批量迁移（50 篇正式文章；草稿、缺 title/date、Hugo/shortcode 说明文跳过；详见 `migration-report.md`）
- ⏸ Expressive Code 主题 override（暂不启用；当前使用默认代码块，保留复制按钮）
- ✅ GitHub Actions workflow（`.github/workflows/deploy.yml`）
- ⏳ GitHub Pages 仓库设置切换到 GitHub Actions Source，并确认首次部署
- ✅ Favicon / OG 默认图占位
- ✅ 第一篇验证文章（dummy）已落地，待替换为正式内容
- ✅ `ARCHITECTURE.md` 日常修改指南已落地
